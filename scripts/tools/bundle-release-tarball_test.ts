...
import { gnuTarCreateArgs } from '../lib/archive-path-for-gnu-tar.ts'
...
Deno.test('gnuTarCreateArgs matches what writeReleaseTarball passes to tar', () => {
  assertEquals(
    gnuTarCreateArgs('/tmp/out/a.tar.gz', 'comment-checker'),
    ['--force-local', '-czf', '/tmp/out/a.tar.gz', 'comment-checker'],
  )
})
