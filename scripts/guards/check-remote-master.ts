...
/**
 * Pre-push gate: refuse when a pushed branch ref does not contain remote `master`.
 *
 * Local `master` is not consulted — it can lag. Query the remote tip; require
...
const shape = (verdict: Verdict): GateOutput => {
  ...
  return {
    code: 1,
    lines: [
      `pre-push: behind remote master (${verdict.sha}).`,
      ...verdict.offenders.map((ref: typeof Behind.infer) => `  ${ref.name} ${ref.sha}`),
      'Fetch and rebase onto remote master before pushing.',
    ],
  }
}
...
const refuseQuery = (remote: string, detail: string): GateOutput => ({
  code: 1,
  lines: [`pre-push: could not query ${remote} master — ${detail}`],
})

const refuseFetch = (remote: string, detail: string): GateOutput => ({
  code: 1,
  lines: [`pre-push: could not fetch ${remote} master — ${detail}`],
})
...
const ensureObject = async (
  ...
  const byRef = await git(['fetch', '--quiet', remote, 'refs/heads/master'])
...
const run = async (
  ...
  const ls = await git(['ls-remote', remote, 'refs/heads/master'])
